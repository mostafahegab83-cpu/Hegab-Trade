/* StockPilot — Battery & Tire Inventory (Firebase Firestore backed) */
...
const adjustmentsCol = collection(db, "adjustments");

// In-memory mirror of Firestore (kept in sync via onSnapshot)
let state = {
  records: [],
  adjustments: [],
...
function rebuildSuggestions(){
  const types = ['Battery', 'Tire'];
  let brands = [], sizes = [], suppliers = [];
  for(const r of state.records){
    if(r.productType) types.includes(r.productType) || types.push(r.productType);
    brands = uniqAdd(brands, r.brand);
    sizes = uniqAdd(sizes, r.size);
    if(r.supplier) suppliers = uniqAdd(suppliers, r.supplier);
  }
...
async function addRecord(rec){
  ...
  const payload = {
    id,
    productType: rec.productType || 'Battery',
    brand: rec.brand || '',
    size: rec.size || '',
  ...
  try{
    ...
    toast('Failed to save (check Firestore rules)', 'error');
  }
}
...
async function deleteRecord(id){
  ...
  try{
    ...
    // best-effort: remove related adjustments
    const related = state.adjustments.filter(a => a.recordId === id);
    for(const a of related){
      try{ await deleteDoc(doc(db, "adjustments", a.id)); } catch(_){}
    }
  } catch(e){
...
async function addAdjustment(a){
  const id = uid();
  try{
    ...
    console.error('addAdjustment failed', e);
  }
}
window.deleteRecord = deleteRecord; // for inline onclick
...
onSnapshot(recordsCol, snap => {
  ...
  recordsLoaded = true;
  rebuildSuggestions();
  if(recordsLoaded && adjustmentsLoaded) setStatus('Connected — data syncs across devices', true);
  render();
}, err => {
...
onSnapshot(adjustmentsCol, snap => {
  ...
  snap.forEach(d => arr.push(d.data()));
  arr.sort((a,b) => (b.date || '').localeCompare(a.date || ''));
  state.adjustments = arr;
  adjustmentsLoaded = true;
  if(recordsLoaded && adjustmentsLoaded) setStatus('Connected — data syncs across devices', true);
  render();
}, err => {
  console.error('adjustments listener error', err);
});
...
function renderDashboard(){
  ...
  const group = (rows, key) => {
    const m = new Map();
    rows.forEach(r => m.set(r[key], (m.get(r[key]) || 0) + r.quantity));
    return [...m.entries()];
  };
  const chips = (el, data) => {
    el.innerHTML = data.length
      ? data.map(([n,v]) => `<span class="chip">${n}: <b>${v}</b></span>`).join('')
  ...
  const typeData = [
    ['Battery', bats.reduce((s,r) => s + r.quantity, 0)],
    ['Tire',    tires.reduce((s,r) => s + r.quantity, 0)]
  ];
...
function drawBar(id, labels, data, prefix, setter, prev){
  ...
  setter(new Chart(ctx, {
    ...
    options: {
      ...
      plugins: { legend: { display: false }, tooltip: { callbacks: { label: c => ` ${prefix} ${fmt(c.parsed.y)}` } } },
...
function drawDoughnut(id, labels, data, setter, prev){
  ...
  setter(new Chart(ctx, {
    type: 'doughnut',
    data: { labels, datasets: [{ data, backgroundColor: ['#2563eb','#16a34a','#f59e0b','#a855f7','#ef4444'] }] },
...
/* -------- Inventory page -------- */
let invSort = { key: 'date', dir: 'desc' }, invPage = 0;
const PAGE_SIZE = 10;
function renderInventory(){
  ...
  rows.sort((a,b) => {
    ...
    if(av < bv) return invSort.dir === 'asc' ? -1 : 1;
    if(av > bv) return invSort.dir === 'asc' ? 1 : -1;
    return 0;
  });
...
document.querySelectorAll('#invTable th[data-sort]').forEach(th => th.addEventListener('click', () => {
  ...
  if(invSort.key === k) invSort.dir = invSort.dir === 'asc' ? 'desc' : 'asc';
  else { invSort.key = k; invSort.dir = 'asc'; }
  renderInventory();
}));
...
function renderInvoicePage(){
  ...
  cont.innerHTML = lines.map((l, i) => `
    <div class="line-item">
      <div><label>Type</label>${ac(`l-type-${i}`, l.productType)}</div>
      <div><label>Brand</label>${ac(`l-brand-${i}`, l.brand, 'ACDELCO')}</div>
      <div><label>Size / Model</label>${ac(`l-size-${i}`, l.size, 'TD70')}</div>
  ...
  wireAC('invSupplier', state.suppliers, () => {}, true);
  updTotal();
}
function ac(id, val, ph = ''){
...
function updTotal(){
  const t = lines.reduce((s, l) => s + l.price * l.quantity, 0);
  document.getElementById('invTotal').textContent = 'EGP ' + fmt(t);
}
...
window.lines = lines;
window.updTotal = updTotal;

/* -------- Adjustments -------- */
function renderAdjust(){
  if(!document.getElementById('page-adjust').classList.contains('active')) return;
  const sel = document.getElementById('adjRecord');
  sel.innerHTML = '<option value="">Choose item…</option>' + state.records.map(r =>
    `<option value="${r.id}">${r.productType} · ${r.brand} · ${r.size} (qty ${r.quantity})</option>`
  ).join('');
...
async function applyAdjust(){
  ...
  await updateRecord(id, { quantity: newQty });
  await addAdjustment({
    recordId: id,
    delta,
    reason: document.getElementById('adjReason').value || mode,
    date: new Date().toISOString()
  });
  document.getElementById('adjReason').value = '';
  document.getElementById('adjAmount').value = 1;
  toast(`Stock adjusted (${delta > 0 ? '+' : ''}${delta})`);
}
...
function exportExcel(){
  const data = state.records.map(r => ({
    ProductType: r.productType, Brand: r.brand, Size: r.size,
    Price: r.price, Quantity: r.quantity, TotalValue: r.price * r.quantity,
    InvoiceNumber: r.invoiceNumber, Supplier: r.supplier || '',
...
document.getElementById('importFile').addEventListener('change', async e => {
  ...
  reader.onload = async ev => {
    ...
    let n = 0;
    for(const r of rows){
      const pt = r.ProductType || r.productType;
      const br = r.Brand || r.brand;
      const sz = r.Size || r.size;
      if(!pt || !br || !sz) continue;
      await addRecord({
